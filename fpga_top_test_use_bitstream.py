...
@cocotb.test()
async def test_fpga_top_8x8(dut):
	...
	# dut.fpga_in[0] <= 0
	# B
	dut.fpga_in[7] <= test_in_B
	# A
	dut.fpga_in[8] <= test_in_A
	# dut.fpga_in[3] <= 0
	

	await Timer(100, units='ps')

	result_C = dut.fpga_out[6]
	result_D = dut.fpga_out[9]
	print("fpga_in:", dut.fpga_in)
	print("fpga_out:", dut.fpga_out)


	assert result_C == test_in_A | test_in_B, "port 6 (C) mismatch"
	assert result_D == test_in_A & test_in_B, "port 9 (D) mismatch"
